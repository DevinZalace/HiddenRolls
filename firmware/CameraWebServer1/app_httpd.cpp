...
// Stream camera frames continuously as an MJPEG response for the companion app.
// Each loop iteration captures a frame, formats it for transmission, and sends
// it as a chunked HTTP payload.
...
// Attach the optional flash LED to PWM so it can be used as a status light or
// camera illumination indicator when the board supports it.
void setupLedFlash() {
#if defined(LED_GPIO_NUM)
